...
import { List, ArrowUp, ChevronRight, BookOpen, Layers } from "lucide-react";
...
export const TableOfContents: React.FC<TableOfContentsProps> = ({
  ...
  return (
    <>
      {/* 데스크톱 고정 목차 (Sticky Sidebar) */}
      <aside 
        id="guide-reader-toc-sidebar"
        className={`w-full bg-white/95 backdrop-blur-md rounded-2xl border border-slate-200/90 p-5 shadow-xs transition-all ${className}`}
      >
        {/* 헤더 & 읽기 진행률 바 */}
        ...
        <nav 
          ref={navContainerRef}
          className="mt-3.5 max-h-[calc(100vh-280px)] overflow-y-auto pr-1 space-y-1 text-xs scroll-smooth"
        >
          {items.map((item, idx) => {
            ...
            return (
              <button
                ...
                title={item.text}
                className={`w-full text-left rounded-lg py-2 px-2.5 flex items-center space-x-2 group cursor-pointer transition-colors duration-300 ease-in-out ${
                  isH2 ? "font-semibold" : "pl-5 font-normal text-slate-600"
                } ${
                  isActive 
              ...
              >
                <span className={`shrink-0 transition-colors duration-300 ${
                  ...
                  {isH2 ? (
                    <span className={`inline-block w-1.5 h-1.5 rounded-full transition-colors duration-300 ${isActive ? "bg-blue-600 scale-110" : "bg-slate-400"}`} />
                  ) : (
      ...
      {/* 모바일 플로팅 목차 토글 버튼 및 모달/드롭다운 */}
      <div className="xl:hidden fixed bottom-6 right-6 z-40">
        <button
          onClick={() => setIsMobileOpen(!isMobileOpen)}
          className="flex items-center space-x-2 bg-slate-900 hover:bg-slate-800 text-white px-4 py-3 rounded-full shadow-lg border border-slate-700 font-bold text-xs transition-transform active:scale-95 cursor-pointer"
          aria-label="목차 열기"
        >
        ...
        {isMobileOpen && (
          <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-xs flex items-end sm:items-center justify-center p-4">
            <div className="bg-white w-full max-w-md rounded-3xl p-6 shadow-2xl space-y-4 max-h-[80vh] flex flex-col animate-in fade-in slide-in-from-bottom-6">
              <div className="flex items-center justify-between border-b border-slate-100 pb-3">
                ...
                <button
                  ...
                  className="p-1 rounded-lg text-slate-400 hover:text-slate-700 hover:bg-slate-100 text-xs font-bold"
                >
                  닫기
                </button>
              </div>

              <div className="overflow-y-auto flex-1 space-y-1 text-xs pr-1">
                {items.map((item, idx) => {
                  const isActive = activeId === item.id;
                  const isH2 = item.level === 2;
                  return (
                    <button
                      ...
                      className={`w-full text-left py-2.5 px-3 rounded-xl transition-colors duration-300 ease-in-out flex items-center space-x-2 ${
                        isH2 ? "font-bold text-slate-900" : "pl-6 text-slate-600 font-normal"
                      } ${
                        isActive ? "bg-blue-50 text-blue-700 font-bold" : "hover:bg-slate-50"
                      }`}
                    >
              ...
              <div className="pt-2 border-t border-slate-100 flex items-center justify-between text-xs">
                <button
                  ...
                  className="py-2 px-3 bg-slate-100 hover:bg-slate-200 text-slate-700 font-semibold rounded-xl flex items-center space-x-1"
                >
                  <ArrowUp className="w-3.5 h-3.5" />
                  <span>맨 위로</span>
                </button>
                <span className="text-slate-400 font-mono">진행률 {Math.round(progress)}%</span>
              </div>
            </div>
          </div>
        )}
