...
// Helper function to get Google API indexing client
function getIndexingClient() {
  const credPath = path.join(process.cwd(), "google-indexing-credentials.json");
  let auth = null;
...
// Google Indexing API 설정 상태 체크 및 히스토리 조회
app.get("/api/indexing/status", (req, res) => {
  const credPath = path.join(process.cwd(), "google-indexing-credentials.json");
  const isConfigured = fs.existsSync(credPath) || !!process.env.GOOGLE_INDEXING_CREDENTIALS;
  ...
  const historyPath = path.join(process.cwd(), "indexing-history.json");
  let historyList: any[] = [];
  ...
  res.json({
    isConfigured,
    clientEmail: clientEmail ? `${clientEmail.substring(0, 15)}...${clientEmail.slice(-15)}` : "",
    history: historyList
  });
});

// Google Indexing API 자격 증명 주입 (JSON 파일 저장)
app.post("/api/indexing/credentials", (req, res) => {
  const { credentialsJson } = req.body;
  if (!credentialsJson) {
    return res.status(400).json({ error: "자격 증명 JSON 내용이 실리지 않았습니다." });
  }

  try {
    const credObj = JSON.parse(credentialsJson);
    if (!credObj.client_email || !credObj.private_key) {
      return res.status(400).json({ error: "유효하지 않은 서비스 계정 JSON입니다. client_email 및 private_key 필드가 반드시 포함되어야 합니다." });
    }

    const credPath = path.join(process.cwd(), "google-indexing-credentials.json");
    fs.writeFileSync(credPath, JSON.stringify(credObj, null, 2), "utf-8");

    return res.json({
      status: "success",
      message: "Google Indexing API용 서비스 계정 키 파일이 안전하게 주입되었습니다.",
      clientEmail: credObj.client_email
    });
  } catch (err: any) {
    return res.status(400).json({ error: `JSON 파싱 실패: ${err.message}` });
  }
});

// Google Indexing API 자격 증명 제거
app.delete("/api/indexing/credentials", (req, res) => {
  const credPath = path.join(process.cwd(), "google-indexing-credentials.json");
  if (fs.existsSync(credPath)) {
    try {
      fs.unlinkSync(credPath);
      return res.json({ status: "success", message: "Google Indexing API 자격증명 파일이 미적용화(삭제) 되었습니다." });
    } catch (e: any) {
      return res.status(500).json({ error: `삭제 실패: ${e.message}` });
    }
  }
  return res.json({ status: "success", message: "삭제할 설정 파일이 없습니다." });
});

// Google Indexing API 게시/요청 엔드포인트
app.post("/api/indexing/publish", async (req, res) => {
  const { url, type = "URL_UPDATED" } = req.body;
  if (!url) {
    return res.status(400).json({ error: "지정한 URL이 존재하지 않습니다." });
  }

  const auth = getIndexingClient();
  if (!auth) {
    return res.status(400).json({ 
      error: "Google Indexing API 자격 증명이 주입되지 않았습니다. 서비스 계정 키값(JSON)을 먼저 등록해 주세요." 
    });
  }
  ...
  try {
    ...
    fs.writeFileSync(historyPath, JSON.stringify(historyList.slice(0, 100), null, 2));
    ...
    console.error("[Google Indexing API Core Error]:", err);
    ...
    const logItem = {
      ...
      timestamp: new Date().toISOString(),
      status: "error",
      error: err.message || "구글 Indexing API 통신 불안정 상태입니다."
    };
    historyList.unshift(logItem);
    fs.writeFileSync(historyPath, JSON.stringify(historyList.slice(0, 100), null, 2));

    return res.status(500).json({
      error: "Google Indexing API 호출에 실패했습니다.",
